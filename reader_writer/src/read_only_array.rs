    ...
    T::Args: Clone,
{
    ...
    pub fn split_off(&mut self, at: usize) -> RoArray<'r, T> {
        if at > self.length {
            panic!(
                "`at` ({}) cannot be > the array's length ({}).",
                at, self.length
            )
        };
    ...
    pub fn get(&self, at: usize) -> Option<T> {
        let fixed_size =
            T::fixed_size().expect("Array::get should only be called for Ts that are fixed size.");
        if at >= self.length {
            None
        } else {
            Some(
                self.data_start
                    .offset(at * fixed_size)
                    .read(self.t_args.clone()),
            )
        }
    }
