...
use reader_writer::{
    generic_array::GenericArray, typenum::*, CStr, Readable, Reader, RoArray, WithRead, Writable,
};

use crate::{bnr::Bnr, pak::Pak, thp::Thp};
...
impl GcDisc<'_> {
        ...
        N: ProgressNotifier,
    {
        ...
        let total_size =
            (RawFstEntry::fixed_size().unwrap() * raw_fst.len()) + header_size + file_system_size;
        notifier.notify_total_bytes(total_size);

        let main_dol_offset = raw_fst
            .iter()
            .find(|e| e.name.to_bytes() == "default.dol".as_bytes())
            .map(|e| e.raw_entry.offset)
            .ok_or_else(|| {
                io::Error::new(io::ErrorKind::Other, "Couldn't find default.dol".to_owned())
            })?;
...
impl<'r> FstEntry<'r> {
    ...
    fn write_files<W, N>(
        writer: &mut W,
        notifier: &mut N,
        fst_entries: &[WrappedFstEntry],
    ) -> io::Result<()>
    where
        W: Write,
        N: ProgressNotifier,
    {
        let mut entries: Vec<_> = fst_entries
            .iter()
            .filter(|e| !e.raw_entry.is_folder())
            .collect();
        entries.sort_by(|l, r| l.raw_entry.offset.cmp(&r.raw_entry.offset));

        let mut entries_and_zeroes: Vec<_> = entries[0..entries.len() - 1]
            .iter()
            .zip(entries[1..].iter())
            .map(|(e1, e2)| {
                (
                    *e1,
                    e2.raw_entry.offset - (e1.raw_entry.offset + e1.raw_entry.length),
                )
            })
            .collect();
...
impl<'r> FstEntry<'r> {
    ...
    pub fn guess_kind(&mut self) {
        ...
        if ext == *b"pak" {
            *file = match file {
                FstEntryFile::Unknown(ref reader) => FstEntryFile::Pak(reader.clone().read(())),
        ...
        if ext == *b"thp" {
            *file = match file {
                FstEntryFile::Unknown(ref reader) => FstEntryFile::Thp(reader.clone().read(())),
        ...
        if ext == *b"bnr" {
            *file = match file {
                FstEntryFile::Unknown(ref reader) => FstEntryFile::Bnr(reader.clone().read(())),
