use std::{borrow::Cow, io, iter, vec};

use reader_writer::{generic_array::GenericArray, typenum::*, LazyArray, Reader, Writable};
...
impl<'a> BinaryPatcher<'a> {
    ...
    fn build_ref(&self) -> PatchedBinary {
        let mut segments = vec![];
...
impl<'a> DolSegment<'a> {
    fn new(seg: &structs::DolSegment<'a>) -> DolSegment<'a> {
        ...
        if seg.load_addr == 0 {
            ...
            DolSegment::Patched(seg.load_addr, BinaryPatcher::new(bytes))
        }
    }

    fn is_empty(&self) -> bool {
        matches!(self, DolSegment::Empty)
    }

    fn len(&self) -> u32 {
        match self {
            DolSegment::Patched(_, patcher) => patcher.len() as u32,
            DolSegment::New(_, bytes) => bytes.len() as u32,
            DolSegment::Empty => 0,
        }
    }
    ...
    fn addr(&self) -> u32 {
        match self {
            DolSegment::Patched(addr, _) => *addr,
            DolSegment::New(addr, _) => *addr,
            DolSegment::Empty => 0,
        }
    }
}
...
impl<'a> DolPatcher<'a> {
    ...
    pub fn iter_section_addrs_and_sizes(&self) -> impl Iterator<Item = (u32, u32)> + '_ {
    ...
    fn check_for_overlapping_segment(&self, addr: u32, len: u32) -> Result<(), String> {
        ...
        for (i, seg) in self.data_segments.iter().enumerate() {
            if check_overlap(seg) {
                Err(format!(
                    "New segment at {:x} overlaps with data segment {} at {:x}",
                    addr,
                    i,
                    seg.addr()
                ))?
    ...
    pub fn add_data_segment(
        ...
        if bytes.len() & 0x1f != 0 {
            Err(format!(
                "Invalid length for new data ({}) - not 32 byte aligned",
                bytes.len()
            ))?;
        }
        ...
        let seg = self
            ...
            .ok_or_else(|| "Insufficent room for new data segment".to_string())?;
        *seg = DolSegment::New(addr, bytes);
        Ok(self)
    }
    ...
    pub fn add_text_segment(
        ...
        if bytes.len() & 0x1f != 0 {
            Err(format!(
                "Invalid length for new text ({}) - not 32 byte aligned",
                bytes.len()
            ))?;
        }
        self.check_for_overlapping_segment(addr, bytes.len() as u32)?;
        let seg = self
            ...
            .iter_mut()
            .find(|seg| seg.is_empty())
            .ok_or_else(|| "Insufficent room for new text segment".to_string())?;
        *seg = DolSegment::New(addr, bytes);
        Ok(self)
    }

    pub fn add_ppcasm_text_segment<A, L>(
        &mut self,
        asm: &ppcasm::AsmBlock<A, L>,
    ) -> Result<&mut Self, String>
    where
        A: AsRef<[u32]>,
    {
    ...
    pub fn patch(&mut self, start: u32, data: Cow<'a, [u8]>) -> Result<&mut Self, String> {
        ...
        let (addr, patcher) = match matching_seg {
            Some(DolSegment::Patched(addr, patcher)) => (addr, patcher),
            Some(DolSegment::New(_, _)) => {
                Err("Patches may not be applied to new segments".to_owned())?
            }
            Some(DolSegment::Empty) => unreachable!(),
...
impl reader_writer::WithRead for DolPatcher<'_> {
    ...
    fn with_read(
        ...
        let mut data_segment_refs =
            GenericArray::<_, U11>::from_exact_iter(&self.data_segments).unwrap();
        data_segment_refs.sort_by_key(|seg| {
            if seg.is_empty() {
                0xFFFFFFFF
            } else {
                seg.addr()
        ...
        let mut text_segment_refs =
            GenericArray::<_, U7>::from_exact_iter(&self.text_segments).unwrap();
        text_segment_refs.sort_by_key(|seg| {
            if seg.is_empty() {
                0xFFFFFFFF
            } else {
                seg.addr()
        ...
        let segment_refs: GenericArray<_, U18> = text_segment_refs
            .iter()
        ...
        let iter = segment_refs.into_iter().filter_map(|seg| match seg {
            DolSegment::Patched(_, patcher) => Some(patcher.build_ref()),
            DolSegment::New(_, bytes) => Some(BinaryPatcher::new(&bytes[..]).build()),
            DolSegment::Empty => None,
        });
